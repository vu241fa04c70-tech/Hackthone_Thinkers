import React, { useState, useEffect } from 'react';
import { Check, Sparkles, Volume2, ArrowRight, UserPlus, UserCheck, ChevronLeft, MapPin, Sprout } from 'lucide-react';
import { useLanguage } from '../localization/LanguageContext';
import { speakText } from '../utils/voiceUtils';

export default function LanguageSelectionScreen({ onConfirm }) {
  const { lang, setLanguage, t } = useLanguage();
  const [step, setStep] = useState(1); // 1: Language Selection, 2: Farmer Registration / Login Form

  const [farmersList, setFarmersList] = useState([]);
  const [selectedFarmerId, setSelectedFarmerId] = useState('new');

  // Form State for creating a new farmer profile
  const [formData, setFormData] = useState({
    farmer_name: '',
    main_crop: 'Paddy',
    district: 'Guntur',
    village: 'Mangalagiri',
    state: 'Andhra Pradesh',
    acreage: 2.5,
    phone: ''
  });

  useEffect(() => {
    fetch('/api/farmers')
      .then(res => res.json())
      .then(data => setFarmersList(data))
      .catch(() => {});
  }, []);

  const languages = [
    { id: 'te', mainLabel: 'తెలుగు', subLabel: 'Telugu', flag: '🌾' },
    { id: 'hi', mainLabel: 'हिन्दी', subLabel: 'Hindi', flag: '🇮🇳' },
    { id: 'en', mainLabel: 'English', subLabel: 'English', flag: '🌐' }
  ];

  const crops = [
    { id: 'Paddy', labelTe: 'వరి (Paddy / Rice)', labelEn: 'Paddy (Rice)' },
    { id: 'Tomato', labelTe: 'టమాటా (Tomato)', labelEn: 'Tomato' },
    { id: 'Chilli', labelTe: 'మిరప (Chilli)', labelEn: 'Chilli' },
    { id: 'Cotton', labelTe: 'పత్తి (Cotton)', labelEn: 'Cotton' },
    { id: 'Wheat', labelTe: 'గోధుమ (Wheat)', labelEn: 'Wheat' },
    { id: 'Potato', labelTe: 'బంగాళాదుంప (Potato)', labelEn: 'Potato' }
  ];

  const handleSelectLang = (lId) => {
    setLanguage(lId);
    speakText(
      lId === 'te' 
        ? 'తెలుగు భాష ఎంచుకున్నారు. నమస్కారం!' 
        : (lId === 'hi' ? 'हिंदी भाषा चुनी गई।' : 'English language selected.'),
      lId
    );
  };

  const handleFarmerSelect = (fId) => {
    setSelectedFarmerId(fId);
    if (fId !== 'new') {
      const existing = farmersList.find(f => f.farmer_id === fId);
      if (existing) {
        setFormData({
          farmer_name: existing.farmer_name,
          main_crop: existing.main_crop || 'Paddy',
          district: existing.district || 'Guntur',
          village: existing.village || 'Village',
          state: existing.state || 'Andhra Pradesh',
          acreage: existing.acreage || 2.5,
          phone: existing.phone || ''
        });
      }
    } else {
      setFormData({
        farmer_name: '',
        main_crop: 'Paddy',
        district: 'Guntur',
        village: 'Mangalagiri',
        state: 'Andhra Pradesh',
        acreage: 2.5,
        phone: ''
      });
    }
  };

  const handleCompleteSetup = async (e) => {
    e?.preventDefault();

    let finalProfile = { ...formData };
    if (!finalProfile.farmer_name.trim()) {
      finalProfile.farmer_name = lang === 'te' ? 'రమేష్ గారూ' : 'Ramesh Bhai';
    }

    try {
      // Save profile to backend
      await fetch('/api/farmers', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(finalProfile)
      });
    } catch (err) {}

    // Save profile locally
    localStorage.setItem('kisan_farmer_profile', JSON.stringify(finalProfile));
    speakText(
      lang === 'te' 
        ? `నమస్కారం ${finalProfile.farmer_name}! మీ అకౌంట్ సిద్ధమైంది.` 
        : `Welcome ${finalProfile.farmer_name}! Your farmer account is ready.`,
      lang
    );

    if (onConfirm) onConfirm();
  };

  return (
    <div className="min-h-screen bg-[#070a12] text-slate-100 flex items-center justify-center p-4 sm:p-6 selection:bg-emerald-500 selection:text-slate-950 font-['Plus_Jakarta_Sans',sans-serif]">
      <div className="bg-slate-900/95 border-2 border-emerald-500/50 p-6 sm:p-10 rounded-3xl max-w-xl w-full shadow-2xl space-y-8 backdrop-blur-xl">
        
        {/* Header Branding */}
        <div className="text-center space-y-2">
          <div className="w-16 h-16 rounded-3xl bg-gradient-to-tr from-emerald-500 via-teal-400 to-cyan-500 border border-emerald-400/40 flex items-center justify-center text-3xl mx-auto shadow-xl shadow-emerald-500/20">
            🌾
          </div>
          <h1 className="text-2xl sm:text-3xl font-black text-slate-100 tracking-tight">
            🌾 Kisan Mitra (కిసాన్ మిత్ర)
          </h1>
          <p className="text-xs font-bold text-emerald-400 uppercase tracking-widest">
            {step === 1 ? 'Step 1 of 2 • Language Selection' : 'Step 2 of 2 • Farmer Account Profile'}
          </p>
        </div>

        {/* STEP 1: LANGUAGE SELECTION */}
        {step === 1 && (
          <div className="space-y-6">
            <div className="text-center">
              <h2 className="text-lg font-black text-slate-200">
                దయచేసి మీ భాషను ఎంచుకోండి / Choose your language
              </h2>
            </div>

            <div className="space-y-3">
              {languages.map((l) => {
                const isSelected = lang === l.id;
                return (
                  <button
                    key={l.id}
                    onClick={() => handleSelectLang(l.id)}
                    className={`w-full p-4 rounded-2xl border-2 flex items-center justify-between cursor-pointer transition-all ${
                      isSelected
                        ? 'bg-gradient-to-r from-emerald-950/90 to-teal-950/90 border-emerald-500 text-emerald-300 shadow-xl shadow-emerald-500/20 scale-[1.02]'
                        : 'bg-slate-950/90 border-slate-800 text-slate-300 hover:border-slate-700'
                    }`}
                  >
                    <div className="flex items-center gap-4 text-left">
                      <span className="text-3xl">{l.flag}</span>
                      <div>
                        <div className="text-lg font-black">{l.mainLabel}</div>
                        <div className="text-xs font-bold text-slate-400">{l.subLabel}</div>
                      </div>
                    </div>

                    <div className="flex items-center gap-3">
                      <button
                        type="button"
                        onClick={(e) => {
                          e.stopPropagation();
                          speakText(l.mainLabel, l.id);
                        }}
                        className="p-2 rounded-xl bg-slate-900 hover:bg-slate-800 text-emerald-400 border border-slate-800"
                        title="Listen voice preview"
                      >
                        <Volume2 className="w-4 h-4" />
                      </button>
                      {isSelected && (
                        <div className="w-6 h-6 rounded-full bg-emerald-500 text-slate-950 flex items-center justify-center font-black shrink-0">
                          <Check className="w-4 h-4" />
                        </div>
                      )}
                    </div>
                  </button>
                );
              })}
            </div>

            <button
              onClick={() => setStep(2)}
              className="w-full py-4 rounded-2xl bg-gradient-to-r from-emerald-500 via-teal-400 to-cyan-500 text-slate-950 font-black text-lg shadow-xl shadow-emerald-500/30 flex items-center justify-center gap-2 cursor-pointer transition-transform hover:scale-[1.02]"
            >
              <span>{lang === 'te' ? 'తరువాత (Next) →' : 'Next Step →'}</span>
              <ArrowRight className="w-5 h-5" />
            </button>
          </div>
        )}

        {/* STEP 2: FARMER ACCOUNT CREATION / SELECTION */}
        {step === 2 && (
          <form onSubmit={handleCompleteSetup} className="space-y-6">
            <div className="flex items-center justify-between">
              <button
                type="button"
                onClick={() => setStep(1)}
                className="text-xs font-bold text-slate-400 hover:text-emerald-400 flex items-center gap-1 cursor-pointer"
              >
                <ChevronLeft className="w-4 h-4" />
                <span>{lang === 'te' ? 'భాష మార్చండి' : 'Change Language'}</span>
              </button>
              <span className="text-xs font-black text-emerald-400">
                {lang === 'te' ? 'అకౌంట్ వివరాలు' : 'Account Details'}
              </span>
            </div>

            {/* Select Existing or Create New */}
            <div className="space-y-2">
              <label className="text-xs font-black text-slate-300">
                {lang === 'te' ? 'ఖాతా ఎంచుకోండి లేదా కొత్త అకౌంట్ సృష్టించండి:' : 'Select Profile or Create New Account:'}
              </label>
              <div className="grid grid-cols-2 gap-2">
                <button
                  type="button"
                  onClick={() => handleFarmerSelect('new')}
                  className={`p-3 rounded-xl border text-xs font-black flex items-center gap-2 cursor-pointer ${
                    selectedFarmerId === 'new'
                      ? 'bg-emerald-500 text-slate-950 border-emerald-400'
                      : 'bg-slate-950 border-slate-800 text-slate-300'
                  }`}
                >
                  <UserPlus className="w-4 h-4" />
                  <span>{lang === 'te' ? '➕ కొత్త ఖాతా' : '➕ Create New'}</span>
                </button>

                {farmersList.slice(0, 3).map((f) => (
                  <button
                    key={f.farmer_id}
                    type="button"
                    onClick={() => handleFarmerSelect(f.farmer_id)}
                    className={`p-3 rounded-xl border text-xs font-black flex items-center gap-2 cursor-pointer truncate ${
                      selectedFarmerId === f.farmer_id
                        ? 'bg-emerald-500 text-slate-950 border-emerald-400'
                        : 'bg-slate-950 border-slate-800 text-slate-300'
                    }`}
                  >
                    <UserCheck className="w-4 h-4 shrink-0" />
                    <span className="truncate">{f.farmer_name}</span>
                  </button>
                ))}
              </div>
            </div>

            {/* Profile Input Fields */}
            <div className="space-y-4">
              <div>
                <label className="text-xs font-bold text-slate-300 block mb-1">
                  {lang === 'te' ? 'రైతు పేరు (Farmer Name):' : 'Farmer Name:'}
                </label>
                <input
                  type="text"
                  required
                  value={formData.farmer_name}
                  onChange={(e) => setFormData({ ...formData, farmer_name: e.target.value })}
                  placeholder={lang === 'te' ? 'ఉదా: రమేష్ గారూ' : 'e.g. Ramesh Bhai'}
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-sm font-bold text-slate-100 placeholder-slate-500 focus:outline-none focus:border-emerald-500"
                />
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-bold text-slate-300 block mb-1">
                    {lang === 'te' ? 'ముఖ్య పంట (Main Crop):' : 'Main Crop:'}
                  </label>
                  <select
                    value={formData.main_crop}
                    onChange={(e) => setFormData({ ...formData, main_crop: e.target.value })}
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-3 text-sm font-bold text-slate-100 focus:outline-none focus:border-emerald-500"
                  >
                    {crops.map(c => (
                      <option key={c.id} value={c.id}>
                        {lang === 'te' ? c.labelTe : c.labelEn}
                      </option>
                    ))}
                  </select>
                </div>

                <div>
                  <label className="text-xs font-bold text-slate-300 block mb-1">
                    {lang === 'te' ? 'జిల్లా (District):' : 'District:'}
                  </label>
                  <input
                    type="text"
                    value={formData.district}
                    onChange={(e) => setFormData({ ...formData, district: e.target.value })}
                    placeholder="Guntur"
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-sm font-bold text-slate-100 placeholder-slate-500 focus:outline-none focus:border-emerald-500"
                  />
                </div>
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-bold text-slate-300 block mb-1">
                    {lang === 'te' ? 'గ్రామం (Village):' : 'Village:'}
                  </label>
                  <input
                    type="text"
                    value={formData.village}
                    onChange={(e) => setFormData({ ...formData, village: e.target.value })}
                    placeholder="Mangalagiri"
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-sm font-bold text-slate-100 placeholder-slate-500 focus:outline-none focus:border-emerald-500"
                  />
                </div>

                <div>
                  <label className="text-xs font-bold text-slate-300 block mb-1">
                    {lang === 'te' ? 'భూమి (Acres):' : 'Land (Acres):'}
                  </label>
                  <input
                    type="number"
                    step="0.5"
                    value={formData.acreage}
                    onChange={(e) => setFormData({ ...formData, acreage: parseFloat(e.target.value) || 2.5 })}
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-sm font-bold text-slate-100 focus:outline-none focus:border-emerald-500"
                  />
                </div>
              </div>
            </div>

            <button
              type="submit"
              className="w-full py-4 rounded-2xl bg-gradient-to-r from-emerald-500 via-teal-400 to-cyan-500 text-slate-950 font-black text-lg shadow-xl shadow-emerald-500/30 flex items-center justify-center gap-2 cursor-pointer transition-transform hover:scale-[1.02]"
            >
              <span>{lang === 'te' ? 'అకౌంట్ ప్రారంభించండి ➔' : 'Start Application ➔'}</span>
            </button>
          </form>
        )}

      </div>
    </div>
  );
}
