...
async def diagnose_plant_id_and_gemini(
    ...
    # Extract dynamic crop-specific remedies generated by CropVisionAgent
    organic = report.immediate_treatment[0] if report.immediate_treatment else "Apply Neem Oil (5ml/L water) or Trichoderma viride bio-fungicide."
    chemical = report.immediate_treatment[1] if len(report.immediate_treatment) > 1 else (report.immediate_treatment[0] if report.immediate_treatment else f"Spray {report.pesticide.name if report.pesticide else 'fungicide'} at recommended dosage.")
    prevention = report.prevention_tips[0] if report.prevention_tips else "Maintain field hygiene and proper plant spacing."
    ...
    sc_name = "Solanum lycopersicum"
    for key, val in scientific_names.items():
    ...
    res = {
        ...
        "cause": report.cause,
        "dosage_note": report.dosage_note,
        "pesticide": {
            "name": report.pesticide.name if report.pesticide else "Recommended Fungicide",
            "dosage_per_acre": report.pesticide.dosage_per_acre if report.pesticide else "600g in 200L water",
            "estimated_cost_inr": report.pesticide.estimated_cost_inr if report.pesticide else 450.0
        } if report.pesticide else None,
        "stage1_source": "Agricultural Vision Neural Engine",
        "stage2_source": "Kisan AI Crop Doctor"
    }
